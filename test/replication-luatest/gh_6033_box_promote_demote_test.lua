...
-- Run box.ctl.promote until server is RAFT leader and limbo owner.
-- Might need to run several promotes because of some error. After this
-- function returns we can be 100% sure, that server considers itself leader.
-- After that wait until server is RW.
...
-- Run box.ctl.demote until server is not RAFT leader.
-- Server might still be RW and/or limbo owner, if elections are turned off.
-- See https://github.com/tarantool/tarantool/issues/6860
...
local function wait_space_empty(server, space)
    luatest.helpers.retrying({}, server.exec, server, function(space)
        if box.space[space] == nil or box.space[space]:len() == 0 then
            return
        end
        error('Not empty yet')
    end, {space})
...
g_common.test_limbo_full_interfering_promote = function(g)
    ...
    local f = promote_start(g.server_1)
    local term = g.server_2:synchro_queue_term()
    promote(g.server_2)
...
g_common.test_fail_limbo_ack_promote = function(g)
    ...
    wait_space_empty(g.server_2, 'test')

    wait_sync(g.cluster.servers)
    promote(g.server_2)
    g.server_2:exec(function() box.space.test:drop() end)
    demote(g.server_2)
    wait_sync(g.cluster.servers)
end
